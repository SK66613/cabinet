/* ====== Конструктор Mini-App · admin.js — обновление 11.12.2025 ====== */
...
function collectBlocksOrder(){
  ...
  // если есть игра — вставим их после menuGrid
  if (!!S.game.code){
...
function makeBlueprint(){
  ...
  const bp = {
    app:   { name, theme:{ brand: color, skin }, subtitle },
...
function makeMiniUrl({ app_id, preview='draft' }){
  const base = location.origin;                   // GitHub Pages / домен
  const api  = API_BASE;
...
function bindControls(){
  ...
  // бренд
  ['brand_name','brand_color','brand_sub','brand_skin','hero_title','hero_cover','hero_align','hero_fit'].forEach(id => on($( '#'+id ), 'input', buildPreviewUrl));
  // меню
  on($('#add_pos'),'click', ()=>{
    const pos = {
